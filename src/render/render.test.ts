...
const model: GraphModel = {
  nodes: [{ id: "ns/apps/Deployment/web", kind: "Deployment", name: "web", ns: "demo", group: "demo",
    icon: "deployment", accent: "#171717", tier: 2, summary: "Deployment", manifest: "kind: Deployment\n" }],
  edges: [], groups: [{ id: "demo", label: "demo" }], warnings: [],
};

test("render produces one self-contained HTML doc containing the model and libs", () => {
  ...
  expect(html).toContain("cytoscape");                     // vendored lib inlined
  expect(html).toContain("__MODEL__");                     // engine wiring present
  expect(html).not.toContain("/*MODEL*/");                 // token replaced
  expect(html).not.toContain("<script src");               // nothing external
});

test("injection-safe: </script>, $& and $$ in model can't break out or corrupt libs", () => {
  const m: GraphModel = { nodes: [{ id: "x", kind: "ConfigMap", name: "c", ns: "d", group: "d",
    icon: "configmap", accent: "#f5a623", tier: 3, summary: "ConfigMap",
    manifest: "data:\n  a: \"</script><b>pwn</b> $& $$USD\"\n" }],
    edges: [], groups: [{ id: "d", label: "d" }], warnings: [] };
  const html = render(m);
  expect(html).not.toContain("</script><b>pwn");   // did NOT break out of the script raw
  expect(html).toContain("\\u003c/script>");        // present only in escaped form
  expect(html).toContain("cytoscape");              // vendor lib survived (split/join kept $&)
});
