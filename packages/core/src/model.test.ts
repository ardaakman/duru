...
test("buildModel({manifests:false}) omits manifest entirely (no stringify)", () => {
  const dep = mk({ uid: "ns/apps/Deployment/d", kind: "Deployment", namespace: "ns", name: "d", raw: { kind: "Deployment", data: { big: "blob" } } });
  const m = buildModel([dep], [], [], { manifests: false });
  expect(m.nodes[0].manifest).toBeUndefined();
  const m2 = buildModel([dep], []);
  expect(m2.nodes[0].manifest).toContain("kind: Deployment"); // default unchanged
});
