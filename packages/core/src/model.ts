...
import { kindMeta, groupOf, KIND_META } from "./kinds-meta.js";
...
export interface BuildModelOpts { manifests?: boolean }

export function buildModel(objects: K8sObject[], edges: Edge[], warnings: string[] = [], opts: BuildModelOpts = {}): GraphModel {
  const withManifests = opts.manifests !== false; // default true (CLI behavior)
  ...
  const nodes: Node[] = objects
    ...
    .map((o) => {
      ...
      return {
        ...
        manifest: withManifests ? yamlStringify(o.raw) : undefined,
        source: o.source,
      };
    });

  // Keep all edges (dedupe, drop self loops).
